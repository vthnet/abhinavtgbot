...
def register_recharge_handlers(dp, bot, users_col, txns_col, crypto_col, settings_col, ADMIN_IDS=None, is_admin=None, get_admin_ids=None):
    ...
    @dp.message(StateFilter(RechargeState.waiting_for_qr), F.photo)
    ...
    @dp.callback_query(F.data.startswith("editpay:"))
    ...
    @dp.message(StateFilter(RechargeState.waiting_for_pay_value))
    ...
    @dp.callback_query(F.data.startswith("toggle_funds:"))
    ...
    async def verify_auto_upi(message: Message, state: FSMContext):
        ...
        # Prepare API URL
        mail = "neetaspirantsair1rank@gmail"
        apppass = "ryimaqxsuvzaxnxi"
    ...
    async def manual_menu(cq: CallbackQuery, state: FSMContext):
        ...
        kb = InlineKeyboardBuilder()
        kb.button(text="Pay now", callback_data="manual_pay:Binance", icon_custom_emoji_id="5217811903685865303")
    ...
    async def manual_pay_details(cq: CallbackQuery, state: FSMContext):
        ...
        await cq.message.edit_text(
            f"{icon} <b>Deposit via crypto</b>\n\n"
            f"🆔 <b>BNB Smart chain:</b> <code>0xCB8a817f27005C947FDB56060c79A289a73Cd39F</code>\n"
            f"💱 <b>Rate:</b> 1 USDT = ₹{live_rate:.2f}\n\n"
            f"⚠️ <b>Instructions:</b>\n"
            f"1. Copy the address above.\n"
            f"2. pay the required amount carefully and take payment confirmation screenshot to share here\n"
            f"3. Click 'I've Paid'after transfer.",
            parse_mode="HTML",
            reply_markup=kb.as_markup()
        )
    ...
    async def approve_txn(cq: CallbackQuery):
        ...
        if txn["status"] != "pending":
            return await cq.answer(f"Already {txn['status']}.", show_alert=True)
    ...
    async def decline_txn(cq: CallbackQuery):
        ...
        txns_col.update_one(
            ...
            {"$set": {
                ...
                "declined_by": cq.from_user.id,
            }},
        )

        # Notify the user who submitted the manual payment proof.
        try:
            await bot.send_message(
                txn["user_id"],
                "❌ <b>Payment Rejected</b>\n\n"
                "Your manual payment proof was declined by an admin.\n"
                "If you believe this was a mistake, please contact support.",
                parse_mode="HTML",
            )
        except Exception as e:
            print(f"Could not notify declined-payment user {txn['user_id']}: {e}")
